<template>
	<div class="menuDiv2">
		<div class="param1">
			...
			<label for="person">启用视频遮挡报警</label>
		</div>
			...
			<div class="checkTime">
				...
				<div class="scaleDiv" v-for="(item,index) in line">
					...
					<div @mousedown.left="mouseDownTime(item)" class="timeLine" id="timeLine">
						<div @click="selectScale(scale)" class="scaleLine" v-bind:class="{scaleLineSele: scale.select}" v-for="scale in item.scaleLine" :style="{width:scale.width+'px',left:scale.start+'px'}">
						</div>
					</div>
...
//  			{"start":0,"width":0,"select":false}	scaleLine内元素的格式
  ...
  methods:{
  	...
  	mouseDownTime(item,event){
		...
		console.log(item.scaleLine);
		var leftMax = 0;	//最大可以拖动的距离，不能与右边的重叠
		for(var i=0;i<item.scaleLine.length;i++){
			leftMax = item.scaleLine[i].start;
			if(item.scaleLine[i].start > disX && item.scaleLine[i].start<leftMax){
				leftMax = item.scaleLine[i].start;
			}
		}
...
//				item.scaleLine = _this.line[_this.copyIndex]["scaleLine"].slice(0);
  	...
  	clickSave(){
  		...
  		this.msgData["Occlusion-Properties"] = this.useData;
  		var sendObj={
		  	"Method" : "SetConfig",
		  	"Page" : "OcclusionDet",
  	...
  	getConfigResult(data){
		if(data["RetCode"] == 0){
    		...
    		this.useData = this.msgData["Occlusion-Properties"];
